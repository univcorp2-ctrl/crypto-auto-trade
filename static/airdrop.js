...
function cardHtml(target) {
  ...
  return `
    <article class="agent-card">
      ...
      <div class="details">
        <div><span>Reward</span><strong>${target.reward_unit}</strong></div>
...
function renderGrid() {
  ...
  const targets = (report.targets || []).filter((target) => {
    ...
    const haystack = `${target.name} ${target.slug} ${target.reward_unit} ${target.mode} ${target.seed_note}`.toLowerCase();
