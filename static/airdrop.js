...
function lifecycleClass(status) {
  ...
  if (status === "CONFLICT" || status === "UNVERIFIED") return "bad";
  return "warn";
}
...
function renderFreshness(data) {
  ...
  let notice = el("staleStatusNotice");
  if (!notice) {
    notice = document.createElement("section");
    notice.id = "staleStatusNotice";
    notice.className = "notice";
    metrics.parentNode.insertBefore(notice, metrics);
  }

  const generated = new Date(data.generated_at);
  const ageMinutes = Number.isNaN(generated.getTime())
    ? Number.POSITIVE_INFINITY
    : (Date.now() - generated.getTime()) / 60000;

  if (ageMinutes < STALE_STATUS_WARNING_MINUTES) {
    notice.hidden = true;
    notice.textContent = "";
    return;
  }
...
function cardHtml(target) {
  ...
  const lifecycle = target.program_lifecycle_status || "REVERIFY";
  const lifecycleLinks = (target.program_lifecycle_sources || []).map((url, index) => `<a href="${url}" target="_blank" rel="noreferrer">Lifecycle ${index + 1} ↗</a>`).join("");
  return `
    <article class="agent-card">
      <div class="agent-top">
        <div>
          ...
          <div class="badges">
            ...
            <span class="badge ${eligibilityClass}">Reward mechanics ${target.api_reward_eligibility || "REVERIFY"}</span>
            <span class="badge ${lifecycleClass(lifecycle)}">Program lifecycle ${lifecycle}</span>
          </div>
        </div>
      ...
      <div class="details">
        ...
        <div><span>API reward mechanics</span><strong>${target.api_reward_eligibility}</strong></div>
        <div><span>Program lifecycle</span><strong>${lifecycle}</strong></div>
        <div><span>Reward rule verified</span><strong>${formatDate(target.reward_rule_verified_at)}</strong></div>
        <div><span>Lifecycle verified</span><strong>${formatDate(target.program_lifecycle_verified_at)}</strong></div>
      ...
      ${target.reward_evidence_note ? `<p class="reason">Reward mechanics: ${target.reward_evidence_note}</p>` : ""}
      ${target.program_lifecycle_note ? `<p class="reason">Program lifecycle: ${target.program_lifecycle_note}</p>` : ""}
      <div class="links">
...
function renderGrid() {
  ...
  const targets = (report.targets || []).filter((target) => {
    ...
    const haystack = `${target.name} ${target.slug} ${target.reward_unit} ${target.mode} ${target.seed_note} ${target.api_reward_eligibility} ${target.program_lifecycle_status}`.toLowerCase();
